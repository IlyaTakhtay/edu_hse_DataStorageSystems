...
### Генерация Hash Diff в Satellites

Hash Diff вычисляется на основе **всех описательных атрибутов**, которые характеризуют состояние сущности. Используется для отслеживания изменений данных:

| Satellite                | Атрибуты состояния (из датасета)          | Hash Diff                                                                                                  |
| ------------------------ | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `sat_location_details`   | `city`, `state`, `region`                 | `MD5(city \|\| '\|' \|\| state \|\| '\|' \|\| region)`                                                     |
| `sat_product_category`   | `category_hk`                             | `MD5(category_hk)`                                                                                         |
| `sat_order_line_metrics` | `sales`, `quantity`, `discount`, `profit` | `MD5(sales::TEXT \|\| '\|' \|\| quantity::TEXT \|\| '\|' \|\| discount::TEXT \|\| '\|' \|\| profit::TEXT)` |

**Принцип:** все атрибуты, описывающие состояние или контекст сущности, включаются в Hash Diff для определения факта изменения данных.
